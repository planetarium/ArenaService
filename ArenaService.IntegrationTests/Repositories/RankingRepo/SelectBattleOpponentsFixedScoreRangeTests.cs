...
public class SelectBattleOpponentsFixedScoreRangeTests : BaseTest
{
    ...
    public async Task SelectBattleOpponentsFixedScoreRangeCorrectly()
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            await CheckRankInRange(seasonId, roundId, participants[0], rankingKey);
            await CheckRankInRange(seasonId, roundId, participants[30], rankingKey);
            await CheckRankInRange(seasonId, roundId, participants[50], rankingKey);
            await CheckRankInRange(seasonId, roundId, participants[70], rankingKey);
            await CheckRankInRange(seasonId, roundId, participants[99], rankingKey);
        }
    }
